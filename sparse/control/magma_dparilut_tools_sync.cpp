/*
    -- MAGMA (version 2.3.0) --
       ...
       @generated from sparse/control/magma_zparilut_tools_sync.cpp, normal z -> d, Wed Nov 15 00:34:25 2017
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    magma_d_matrix L_new={Magma_CSR}, U_new={Magma_CSR};
    // magma_index_t *index_swap;
    double *L_new_val = NULL, *U_new_val = NULL, *val_swap = NULL;
    
    // CHECK( magma_dmtransfer( *L, &L_new, Magma_CPU, Magma_CPU, queue ) );
    // CHECK( magma_dmtransfer( *U, &U_new, Magma_CPU, Magma_CPU, queue ) );
    ...
    for( magma_int_t e=0; e<U->nnz; e++){
        ...
        if( row == col ){ 
            U_new_val[ e ] = MAGMA_D_ONE; // upper triangular has diagonal equal 1
        } else {
