/*
    -- MAGMA (version 2.4.0) --
       ...
       @generated from sparse/src/zparilut_gpu.cpp, normal z -> s, Mon Jun 25 18:24:31 2018
*/
    ...
    magma_queue_t queue)
{
    ...
    for (magma_int_t iters =0; iters<precond->sweeps; iters++) {
        ...
        CHECK(magma_spreselect_gpu(0, &dL, &oneL, queue));
        CHECK(magma_spreselect_gpu(0, &dU, &oneU, queue));
        if (num_rmL>0) {
            magma_s_matrix hLr={Magma_CSR};
            hLr.nnz = oneL.nnz;
            magma_smalloc_cpu( &hLr.val, hLr.nnz );
            magma_sgetvector( oneL.nnz , oneL.dval, 1, hLr.val, 1, queue );
            
            CHECK(magma_sparilut_set_thrs_randomselect_approx(num_rmL, 
                &hLr, 0, &thrsL, queue));
            magma_smfree(&hLr, queue);
            
            //CHECK(magma_sthrsholdselect(10, oneL.nnz, num_rmL, 
            //    oneL.dval, &thrsL, queue));
        } else {
            thrsL = 0.0;
        }
        if (num_rmU>0) {
             magma_s_matrix hUr={Magma_CSR};
             hUr.nnz = oneU.nnz;
             magma_smalloc_cpu( &hUr.val, hUr.nnz );
             magma_sgetvector( oneU.nnz , oneU.dval, 1, hUr.val, 1, queue );
             
             CHECK(magma_sparilut_set_thrs_randomselect_approx(num_rmL, 
                 &hUr, 0, &thrsU, queue));
             magma_smfree(&hUr, queue);
            //CHECK(magma_sthrsholdselect(10, oneU.nnz, num_rmU, 
