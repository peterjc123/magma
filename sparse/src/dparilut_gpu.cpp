/*
    -- MAGMA (version 2.4.0) --
       ...
       @generated from sparse/src/zparilut_gpu.cpp, normal z -> d, Mon Jun 25 18:24:31 2018
*/
    ...
    magma_queue_t queue)
{
    ...
    for (magma_int_t iters =0; iters<precond->sweeps; iters++) {
        ...
        CHECK(magma_dpreselect_gpu(0, &dL, &oneL, queue));
        CHECK(magma_dpreselect_gpu(0, &dU, &oneU, queue));
        if (num_rmL>0) {
            magma_d_matrix hLr={Magma_CSR};
            hLr.nnz = oneL.nnz;
            magma_dmalloc_cpu( &hLr.val, hLr.nnz );
            magma_dgetvector( oneL.nnz , oneL.dval, 1, hLr.val, 1, queue );
            
            CHECK(magma_dparilut_set_thrs_randomselect_approx(num_rmL, 
                &hLr, 0, &thrsL, queue));
            magma_dmfree(&hLr, queue);
            
            //CHECK(magma_dthrsholdselect(10, oneL.nnz, num_rmL, 
            //    oneL.dval, &thrsL, queue));
        } else {
            thrsL = 0.0;
        }
        if (num_rmU>0) {
             magma_d_matrix hUr={Magma_CSR};
             hUr.nnz = oneU.nnz;
             magma_dmalloc_cpu( &hUr.val, hUr.nnz );
             magma_dgetvector( oneU.nnz , oneU.dval, 1, hUr.val, 1, queue );
             
             CHECK(magma_dparilut_set_thrs_randomselect_approx(num_rmL, 
                 &hUr, 0, &thrsU, queue));
             magma_dmfree(&hUr, queue);
            //CHECK(magma_dthrsholdselect(10, oneU.nnz, num_rmU, 
